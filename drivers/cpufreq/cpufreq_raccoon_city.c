 ...
 * Author: AbandonedCart (twistedumbrella@gmail.com)
 */
...
struct raccoon_city_policy {
	struct cpufreq_policy *policy;
	struct raccoon_city_tunables *tunables;
	struct list_head tunables_hook;
};

/* Separate instance required for each CPU */
struct raccoon_city_cpu {
	struct update_util_data update_util;
	struct raccoon_city_policy *ipolicy;
...
static DEFINE_PER_CPU(struct raccoon_city_cpu, raccoon_city_cpu);
...
static struct raccoon_city_tunables *global_tunables;
static DEFINE_MUTEX(global_tunables_lock);

static inline void update_slack_delay(struct raccoon_city_tunables *tunables)
{
	tunables->timer_slack_delay = usecs_to_jiffies(tunables->timer_slack +
						       tunables->sampling_rate);
}

static bool timer_slack_required(struct raccoon_city_cpu *icpu)
{
	struct raccoon_city_policy *ipolicy = icpu->ipolicy;
	struct raccoon_city_tunables *tunables = ipolicy->tunables;
...
static void gov_slack_timer_start(struct raccoon_city_cpu *icpu, int cpu)
{
	struct raccoon_city_tunables *tunables = icpu->ipolicy->tunables;

	icpu->slack_timer.expires = jiffies + tunables->timer_slack_delay;
	add_timer_on(&icpu->slack_timer, cpu);
}

static void gov_slack_timer_modify(struct raccoon_city_cpu *icpu)
{
	struct raccoon_city_tunables *tunables = icpu->ipolicy->tunables;
...
static void slack_timer_resched(struct raccoon_city_cpu *icpu, int cpu,
				bool modify)
{
	struct raccoon_city_tunables *tunables = icpu->ipolicy->tunables;
	unsigned long flags;
...
static unsigned int
freq_to_above_hispeed_delay(struct raccoon_city_tunables *tunables,
			    unsigned int freq)
{
	unsigned long flags;
	unsigned int ret;
...
static unsigned int freq_to_targetload(struct raccoon_city_tunables *tunables,
				       unsigned int freq)
{
	unsigned long flags;
	unsigned int ret;
 ...
 */
static unsigned int choose_freq(struct raccoon_city_cpu *icpu,
...
static u64 update_load(struct raccoon_city_cpu *icpu, int cpu)
{
	struct raccoon_city_tunables *tunables = icpu->ipolicy->tunables;
...
static void eval_target_freq(struct raccoon_city_cpu *icpu)
{
	struct raccoon_city_tunables *tunables = icpu->ipolicy->tunables;
	struct cpufreq_policy *policy = icpu->ipolicy->policy;
	struct cpufreq_frequency_table *freq_table = policy->freq_table;
	...
	if (policy->cur >= tunables->hispeed_freq &&
		...
		trace_cpufreq_raccoon_city_notyet(cpu, cpu_load,
	...
	if (icpu->target_freq == new_freq &&
		...
		trace_cpufreq_raccoon_city_already(cpu, cpu_load,
			icpu->target_freq, policy->cur, new_freq);
		goto exit;
	}

	trace_cpufreq_raccoon_city_target(cpu, cpu_load, icpu->target_freq,
					 policy->cur, new_freq);
...
static void cpufreq_raccoon_city_update(struct raccoon_city_cpu *icpu)
{
	eval_target_freq(icpu);
	slack_timer_resched(icpu, smp_processor_id(), true);
}

static void cpufreq_raccoon_city_idle_end(void)
{
	struct raccoon_city_cpu *icpu = &per_cpu(raccoon_city_cpu,
						smp_processor_id());
	...
	if (icpu->ipolicy) {
		...
		if (time_after_eq(jiffies, icpu->next_sample_jiffies))
			cpufreq_raccoon_city_update(icpu);
	}

	up_read(&icpu->enable_sem);
}

static void cpufreq_raccoon_city_get_policy_info(struct cpufreq_policy *policy,
						unsigned int *pmax_freq,
						u64 *phvt, u64 *pfvt)
{
	struct raccoon_city_cpu *icpu;
	u64 hvt = ~0ULL, fvt = 0;
	unsigned int max_freq = 0, i;

	for_each_cpu(i, policy->cpus) {
		icpu = &per_cpu(raccoon_city_cpu, i);
...
static void cpufreq_raccoon_city_adjust_cpu(unsigned int cpu,
					   struct cpufreq_policy *policy)
{
	struct raccoon_city_cpu *icpu;
	u64 hvt, fvt;
	...
	cpufreq_raccoon_city_get_policy_info(policy, &max_freq, &hvt, &fvt);
	...
	trace_cpufreq_raccoon_city_setspeed(cpu, max_freq, policy->cur);
}

static int cpufreq_raccoon_city_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
...
again:
	...
	for_each_cpu(cpu, &tmp_mask) {
		struct raccoon_city_cpu *icpu = &per_cpu(raccoon_city_cpu, cpu);
		struct cpufreq_policy *policy;
		...
		if (likely(icpu->ipolicy)) {
			...
			cpufreq_raccoon_city_adjust_cpu(cpu, policy);
		}
...
static void cpufreq_raccoon_city_boost(struct raccoon_city_tunables *tunables)
{
	struct raccoon_city_policy *ipolicy;
	struct cpufreq_policy *policy;
	struct raccoon_city_cpu *icpu;
...
static int cpufreq_raccoon_city_notifier(struct notifier_block *nb,
					unsigned long val, void *data)
{
	struct cpufreq_freqs *freq = data;
	struct raccoon_city_cpu *icpu = &per_cpu(raccoon_city_cpu, freq->policy->cpu);
	unsigned long flags;
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_raccoon_city_notifier,
};
...
{									\
	struct raccoon_city_tunables *tunables = to_tunables(attr_set);	\
			   ...
			   size_t count)
{
	...
	if (tunables->boost) {
		trace_cpufreq_raccoon_city_boost("on");
		if (!tunables->boosted)
			cpufreq_raccoon_city_boost(tunables);
	} else {
		tunables->boostpulse_endtime = ktime_to_us(ktime_get());
		trace_cpufreq_raccoon_city_unboost("off");
	}
				...
				size_t count)
{
	...
	trace_cpufreq_raccoon_city_boost("pulse");
	if (!tunables->boosted)
		cpufreq_raccoon_city_boost(tunables);
				...
				const char *buf, size_t count)
{
	...
	tunables->max_inactive_freq_screen_on = val;
				...
				const char *buf, size_t count)
{
	...
	tunables->max_inactive_freq_screen_off = val;
				...
				const char *buf, size_t count)
{
	...
	tunables->max_inactive_freq = val;
...
static struct kobj_type raccoon_city_tunables_ktype = {
	.default_attrs = raccoon_city_attributes,
	.sysfs_ops = &governor_sysfs_ops,
};

static int cpufreq_raccoon_city_idle_notifier(struct notifier_block *nb,
					     unsigned long val, void *data)
{
	if (val == IDLE_END)
		cpufreq_raccoon_city_idle_end();

	return 0;
}

static struct notifier_block cpufreq_raccoon_city_idle_nb = {
...
static struct raccoon_city_governor raccoon_city_gov;

#define CPU_FREQ_GOV_RACCOON_CITY	(&raccoon_city_gov.gov)

static void irq_work(struct irq_work *irq_work)
{
	struct raccoon_city_cpu *icpu = container_of(irq_work, struct
						    raccoon_city_cpu, irq_work);
...
static void update_util_handler(struct update_util_data *data, u64 time,
				unsigned int flags)
{
	struct raccoon_city_cpu *icpu = container_of(data,
					struct raccoon_city_cpu, update_util);
	struct raccoon_city_policy *ipolicy = icpu->ipolicy;
	struct raccoon_city_tunables *tunables = ipolicy->tunables;
	u64 delta_ns;
...
static void gov_set_update_util(struct raccoon_city_policy *ipolicy)
{
	...
	for_each_cpu(cpu, policy->cpus) {
		icpu = &per_cpu(raccoon_city_cpu, cpu);
...
static void icpu_cancel_work(struct raccoon_city_cpu *icpu)
{
...
static struct raccoon_city_policy *
raccoon_city_policy_alloc(struct cpufreq_policy *policy)
{
	struct raccoon_city_policy *ipolicy;
...
static void raccoon_city_policy_free(struct raccoon_city_policy *ipolicy)
{
	kfree(ipolicy);
}

static struct raccoon_city_tunables *
raccoon_city_tunables_alloc(struct raccoon_city_policy *ipolicy)
{
	struct raccoon_city_tunables *tunables;
...
static void raccoon_city_tunables_free(struct raccoon_city_tunables *tunables)
{
	if (!have_governor_per_policy())
...
int cpufreq_raccoon_city_init(struct cpufreq_policy *policy)
{
	struct raccoon_city_policy *ipolicy;
	struct raccoon_city_tunables *tunables;
	int ret;
	...
	ipolicy = raccoon_city_policy_alloc(policy);
	if (!ipolicy)
		return -ENOMEM;
	...
	tunables = raccoon_city_tunables_alloc(ipolicy);
	if (!tunables) {
		ret = -ENOMEM;
		goto free_int_policy;
	}
	...
	ret = kobject_init_and_add(&tunables->attr_set.kobj,
				   &raccoon_city_tunables_ktype,
				   get_governor_parent_kobj(policy), "%s",
				   raccoon_city_gov.gov.name);
	if (ret)
		goto fail;

	/* One time initialization for governor */
	if (!raccoon_city_gov.usage_count++) {
		idle_notifier_register(&cpufreq_raccoon_city_idle_nb);
 ...
 fail:
	...
	raccoon_city_tunables_free(tunables);

 free_int_policy:
	mutex_unlock(&global_tunables_lock);

	raccoon_city_policy_free(ipolicy);
	pr_err("governor initialization failed (%d)\n", ret);

	return ret;
}

void cpufreq_raccoon_city_exit(struct cpufreq_policy *policy)
{
	struct raccoon_city_policy *ipolicy = policy->governor_data;
	struct raccoon_city_tunables *tunables = ipolicy->tunables;
	unsigned int count;
	...
	if (!--raccoon_city_gov.usage_count) {
		cpufreq_unregister_notifier(&cpufreq_notifier_block,
					    CPUFREQ_TRANSITION_NOTIFIER);
		idle_notifier_unregister(&cpufreq_raccoon_city_idle_nb);
	}
...
int cpufreq_raccoon_city_start(struct cpufreq_policy *policy)
{
	struct raccoon_city_policy *ipolicy = policy->governor_data;
	struct raccoon_city_cpu *icpu;
	unsigned int cpu;
...
void cpufreq_raccoon_city_stop(struct cpufreq_policy *policy)
{
	struct raccoon_city_policy *ipolicy = policy->governor_data;
	struct raccoon_city_cpu *icpu;
	unsigned int cpu;
...
void cpufreq_raccoon_city_limits(struct cpufreq_policy *policy)
{
	struct raccoon_city_cpu *icpu;
	unsigned int cpu;
	unsigned long flags;
...
static struct raccoon_city_governor raccoon_city_gov = {
	.gov = {
		...
		.owner			= THIS_MODULE,
		.init			= cpufreq_raccoon_city_init,
		.exit			= cpufreq_raccoon_city_exit,
		.start			= cpufreq_raccoon_city_start,
		.stop			= cpufreq_raccoon_city_stop,
		.limits			= cpufreq_raccoon_city_limits,
	}
};
...
static void cpufreq_raccoon_city_nop_timer(struct timer_list *timers)
{
	/*
...
static int __init cpufreq_raccoon_city_gov_init(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		timer_setup(&icpu->slack_timer, cpufreq_raccoon_city_nop_timer, 0UL);
	}

	spin_lock_init(&speedchange_cpumask_lock);
	speedchange_task = kthread_create(cpufreq_raccoon_city_speedchange_task,
					  NULL, "cfraccoon_city");
...
{
	return CPU_FREQ_GOV_RACCOON_CITY;
}

fs_initcall(cpufreq_raccoon_city_gov_init);
#else
module_init(cpufreq_raccoon_city_gov_init);
#endif

static void __exit cpufreq_raccoon_city_gov_exit(void)
{
	cpufreq_unregister_governor(CPU_FREQ_GOV_RACCOON_CITY);
